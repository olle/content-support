 ...
 *
 * <p>
 * Our use of a {@code mimeType} property, is how we like to classify and structure content. This class is an attempt to
 * define the currently known and supported declarations. By that I mean, since it's basically <i> possible</i> to use
 * whatever string value you want, this is merely a list of the <strong>recommended</strong> ones.
 * </p>
 *
 * <p>
 * Though not strictly IANA or RFC compatible, we have chosen to keep to a structure that expresses <i>some
 ...
 *
 * <p>
 * Our mime type definitions are defined using the following pattern, with optional variations:
 * </p>
 *
 * <pre>
 * <code>
${type}/vnd.content.${name}
${type}/vnd.content.${name};${params}
${type}/vnd.content.${name}+${subtype}
${type}/vnd.content.${name}+${subtype};${params}
   </code>
 * </pre>
 *
 * <p>
 * The constant {@code vnd.content} is our vendor specifier. Which for our purposes is <strong>mandatory</strong>. The
 * {@code type}, {@code subtype} and {@code params} values, try to follow the way that IANA/RFC based media types are
 * declared.
 * </p>
 *
 * <p>
 * What we describe with {@code name} should hopefully have some semantic but still be open for broad use. We always try
 * to define these types with documentation, as a hint for developers.
 * </p>
 *
 * <p>
 * Here's an example of what a mime type could look like: <code>text/vnd.content.appicon+uri;class=small</code>
 * </p>
 *
 * <p>
 * In this example a mime-type <em>parameter</em> is used to mark the content with a <code>class</code>, this is not a
 * specific parameter but an option for the authoring code, as means to extend the specification of the provided
 * content, to the user.
 * </p>
 *
 *
...
public final class MimeType {
     ...
     *
     * <p>
     * For example an email or message subject line, describing the contents.
     * </p>
     */
     ...
     *
     * <p>
     * For example a clarification on some piece of information, such as it's type, structure or classification -
     * currently used to describe the domain type of a search-result.
     * </p>
     */
     ...
     *
     * <p>
     * For example a message body, from a service or machine, that has to be sent and persisted in multiple languages.
     * </p>
     */
     ...
     *
     * <p>
     * Typically provided for the rendering of a size-independent (scalable) image or background. More specifically, the
     * URI to an application's SVG icon resource.
     * </p>
     *
     * <p>
     * NOTE: It is intended for the content author to optionally extend the content with a classification, which may
     * allow for more suitable use by the client. Such classifications are preferably to apply with a mime-type
     * parameter, for example: {@code ;class=small}
     * </p>
     *
     * @since 0.2
     */
     ...
     *
     * <p>
     * Absolutely <strong>no checks or validations made</strong>. The provided mime type string will be used as-is.
     * </p>
     *
     * @param mimeType
     *            value to create
     ...
     *
     * @param mimeTypeValue
     *            string for the mime type to create
     *
